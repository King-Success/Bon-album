...
class EloquentProfileRepository implements ProfileContract
{
    ...
    public function getAllProfiles($albumID){
        $profile_ids = AlbumProfile::where('album_id', $albumID)->pluck('profile_id')->toArray();
        return $profiles = Profile::whereIn('id', $profile_ids)->with(['pictures', 'albums'])->get()->toArray();
    }
    ...
    public function getProfileByID($profileID){
        $profile = Profile::find($profileID); 
        $profile['albums'] = $profile->albums()->get()->toArray();
        return $profile;
    }

    public function getProfileByEmail($profileEmail){
        return Profile::where('email', $profileEmail)->first();
    }

    public function doesProfileExistByEmail($profileEmail){
        return Profile::where('email', $profileEmail)->count()? True : false;
    }
    
    public function doesProfileExistByID($profileID){
        return Profile::where('id', $profileID)? True : false;

    }

    public function isUserExist($profile_id){
        if (Profile::where('id', '=', $profile_id)->exists()) {
            return true;
         }
         return false;
    }
